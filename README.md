...
```text
.
├── README.md                 # プロジェクトの総合説明書
├── mf-portfolio-ranking.png  # README用のスクリーンショット画像
├── dummy-data.js             # 開発・デバッグ用ダミーデータ生成スクリプト
└── src/                      # ★拡張機能本体（配布・読み込み対象）
    ├── manifest.json         # 拡張機能の設定ファイル
    ├── content.js            # データ抽出・描画ロジック
    ├── style.css             # UIデザイン・スタイル定義
    └── icons/                # アイコン資産フォルダ
        ├── icon.svg          # マスターデータ（ベクター）
        ├── 32.png       # ツールバー/管理画面用
        ├── 64.png       # 高解像度ディスプレイ用
        └── 128.png      # ストア/インストール画面用（最重要）
```

本プロジェクトは、拡張機能本体を `src` フォルダに集約しています。

* **`src/`**: 拡張機能本体
* `manifest.json`: ブラウザ設定
* `content.js`: データ抽出・描画ロジック
* `style.css`: UIデザイン定義
* **`icons/`**: 拡張機能用アイコン（32px, 64px, 128px, SVG）


* **`dummy-data.js`**: 開発・デバッグ用ダミーデータ生成スクリプト
* **`mf-portfolio-ranking.png`**: README用スクリーンショット
