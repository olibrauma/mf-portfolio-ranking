const initChart = () => {
    if (typeof assetClassRatio === 'undefined') return;
    if (document.getElementById('custom-bar-chart')) return;

    const grandTotal = assetClassRatio.reduce((sum, item) => sum + item.y, 0);
    ...
    const data = assetClassRatio
        .filter(item => item.y > 0)
        .sort((a, b) => b.y - a.y)
        .map(item => ({
            ...item,
            details: getDetails(idMap[item.name])
        }));

    const chartContainer = document.createElement('div');
    chartContainer.id = 'custom-bar-chart';
    chartContainer.style.cssText = `
        background: #ffffff; padding: 30px; margin: 0 0 40px 0; border-radius: 16px;
        font-family: -apple-system, sans-serif; box-shadow: 0 10px 30px rgba(0,0,0,0.08); color: #333;
    `;
            ...
            <div style="cursor:pointer; user-select:none;" id="btn-${index}">
                <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 8px;">
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <span id="${arrowId}" style="display: inline-block; width: 0; height: 0; border-style: solid; border-width: 5px 0 5px 8.7px; border-color: transparent transparent transparent #ccc; transition: transform 0.2s ease;"></span>
                ...
                <div style="background: #f0f2f5; border-radius: 100px; height: 10px; width: 100%; overflow:hidden;">
                    <div style="width: ${classPercent}%; background: ${item.color}; height: 100%; border-radius: 100px; transition: width 0.6s ease;"></div>
                </div>
            </div>
    ...
    if (mainContainer) mainContainer.prepend(chartContainer);
